...
const handleRemoveQuantity = async (productId) => {
  ...
  const token = localStorage.getItem('auth-token');
  try {
    const response = await fetch(`https://reelbitecinematiccravings1.onrender.com/api/cart/remove`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
const handledeleteQuantity = async (productId) => {
  const token = localStorage.getItem('auth-token');
  try {
    const response = await fetch(`https://reelbitecinematiccravings1.onrender.com/api/cart/remove1`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
